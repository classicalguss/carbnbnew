...
<?php $form = ActiveForm::begin(['options'=>['class'=>'listing-form','onsubmit' => 'switchForm('.$formNum.',this);return false;']]); ?>
	...
	<?= $form->field($model, 'gear_type_id')->radioList(Car::gearArray(),['class'=>'list-inline','item'=>function($index,$label,$name,$checked,$value) {
		if ($checked == 1)
			$checked = 'checked';
	...
	<?= $form->field($model, 'gas_type_id')->radioList(Car::gasArray(),['class'=>'list-inline feature-checkbox','item'=>function($index,$label,$name,$checked,$value) {
		if ($checked == 1)
			$checked = 'checked';
		return '<li><label><img src="'.Yii::$app->params['siteImagesPath'].'/'.$label.'_icon_sm.png"><div>'.$label.'</div><input name='.$name.' value='.$value.' type="radio" '.$checked.'></label></li>';
	}]) ?>
	...
	<?= $form->field($model, 'features')->checkboxList(Car::featuresArray(),['class'=>'list-inline feature-checkbox','item'=>function($index,$label,$name,$checked,$value) {
		if ($checked == 1)
			$checked = 'checked';
		return '<li><label><img src="'.Yii::$app->params['siteImagesPath'].'/'.$label.'_icon_sm.png"><div>'.$label.'</div><input name='.$name.' value='.$value.' type="checkbox" '.$checked.'></label></li>';
	}]) ?>
