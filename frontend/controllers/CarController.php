...
class CarController extends Controller {
	...
	public function actionView($id) {
		...
		$carModel = new Car ();
		$carModel = Car::find ()->joinWith ( 'make', true, 'INNER JOIN' )->joinWith ( 'model', true, 'INNER JOIN' )->joinWith ( 'ratings' )->joinWith ( 'user' )->where ( 'carmake.id = carmodel.make_id' )->andWhere ( [
				'car.id' => $id
		] )->one ();
	...
	public function actionYourCars() {
		...
		$carModel = Car::find ()->joinWith ( 'make', true, 'INNER JOIN' )->joinWith ( 'model', true, 'INNER JOIN' )->where ( 'carmake.id = carmodel.make_id' )->andWhere ( [
				'car.owner_id' => $userId
		] )->orderBy ( 'car.is_published DESC, car.created_at DESC' )->all ();

		$carIds = [ ];
		foreach ( $carModel as $car )
			$carIds [] = $car->id;

		$bookings = Booking::find ()->select ( [
				'car_id',
				'count(*) as count'
		] )->where ( [
				'car_id' => $carIds
				...
				'date_start',
				new Expression ( 'NOW()' )
		] )->groupBy ( 'car_id' )->indexBy ( 'car_id' )->all ();
	...
	public function actionMyDrives() {
		...
		$carModel = Car::find ()->joinWith ( 'make', true, 'INNER JOIN' )->joinWith ( 'model', true, 'INNER JOIN' )->joinWith ( 'user', true, 'INNER JOIN' )->where ( 'carmake.id = carmodel.make_id' )->andWhere ( [
				'car.id' => $carIds
		] )->indexBy ( 'id' )->all ();
	...
	public function actionMyApprovals() {
		...
		$carModel = Car::find ()->joinWith ( 'make', true, 'INNER JOIN' )->joinWith ( 'model', true, 'INNER JOIN' )->joinWith ( 'city', true, 'INNER JOIN' )->where ( 'carmake.id = carmodel.make_id' )->andWhere ( [
				'car.id' => $carIds
		] )->indexBy ( 'id' )->all ();
